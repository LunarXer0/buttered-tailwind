...
const NavBar = () => {
  ...
  const toggleNav = () => {
    navDisplay === "block" ? setNavDisplay("hidden") : setNavDisplay("block");
  };
  ...
  return (
    <nav className="flex items-center justify-between flex-wrap bg-transparent p-6">
      <div className="flex items-center flex-no-shrink text-white mr-6">
        ...
        <span className="font-semibold text-xl tracking-tight">Buttered</span>
      </div>
      ...
      <div
        className={`w-full ${navDisplay} lg:block flex-grow lg:flex lg:items-center lg:w-auto`}
      >
